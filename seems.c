...
#include<ctype.h>
#include<stdlib.h>
// 以cell_num 在主函式中代替static int cell_num
// 處理到 take_action_lottery(用來算出使用者抽到甚麼，並將結果存起來)
// booster 剩餘數位處理
// 每天結束記得加一次免費抽
...
// ************* graphing funcions********************
void graph_main(int *free_choices, int *open_cost, int *n, int *choosen_arr, int *saving, int *boosters, int *price, int *cell_num);
...
int main(){
    ...
    //************************************************************************************
    //graphing(3, &cell_num);
    graph_main(&free_choices, &open_cost, &n, choosen_arr, &saving, &boosters[0], &price, &cell_num);
